(() => {
  ...
  function animalCardTemplate(animal) {
    ...
    let action = `<span class="animal-adopted-message" aria-label="${escapeHtml(animal.name)} ya fue adoptado">🐾 Adoptado</span>`;
    if (!isAdopted) {
    ...
    return `<article class="animal-card${isAdopted ? " animal-card--adopted" : ""}" data-species="${escapeHtml(animal.species)}" data-status="${isAdopted ? "adopted" : "available"}" id="${escapeHtml(animal.id)}">
      <div class="animal-image-wrap">${image}<span class="animal-status${isAdopted ? " animal-status--adopted" : ""}">${isAdopted ? "ADOPTADO" : "En adopción"}</span></div>
  ...
  function initAnimals() {
    ...
    chips.forEach((chip) => {
      chip.addEventListener("click", () => {
        ...
        render(chip.dataset.filter || "available");
      });
    });

    const activeChip = chips.find((chip) => chip.classList.contains("is-active"));
    render(activeChip?.dataset.filter || "available");
  }
