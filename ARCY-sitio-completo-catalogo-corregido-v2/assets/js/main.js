(() => {
  ...
  function animalCardTemplate(animal) {
    ...
    const image = animal.image
      ? `<img src="${escapeHtml(animal.image)}" alt="Fotografía de ${escapeHtml(animal.name)}, ${isAdopted ? "perrito adoptado" : "perrito adoptable"} de ARCY" loading="lazy" width="800" height="1000">`
    ...
    let action = `<button class="button animal-adopted-button" type="button" disabled aria-label="${escapeHtml(animal.name)} ya fue adoptado y no está disponible para adopción">Adoptado · no disponible</button>`;
    if (!isAdopted) {
    ...
    const adoptedSeal = isAdopted ? `<span class="animal-adopted-seal" aria-hidden="true">ADOPTADO</span>` : "";

    return `<article class="animal-card${isAdopted ? " animal-card--adopted" : ""}" data-species="${escapeHtml(animal.species)}" data-status="${isAdopted ? "adopted" : "available"}" id="${escapeHtml(animal.id)}">
      <div class="animal-image-wrap">${image}${adoptedSeal}<span class="animal-status${isAdopted ? " animal-status--adopted" : ""}">${isAdopted ? "ADOPTADO" : "En adopción"}</span></div>
      <div class="animal-card-body">
        ...
        <details class="animal-story">
          <summary>${isAdopted ? "Conocer su historia" : "Conocer su historia"}</summary>
  ...
  function initAnimals() {
    ...
    const render = (filter = "all") => {
      ...
      if (filter === "available") filtered = animals.filter((animal) => animal.status !== "adopted");
      if (filter === "adopted") filtered = animals.filter((animal) => animal.status === "adopted");
      if (filter === "all") filtered = animals;
      if (filter === "Perro" || filter === "Gato") filtered = animals.filter((animal) => animal.species === filter && animal.status !== "adopted");

      if (!filtered.length) {
        grid.innerHTML = `<div class="empty-state"><div><div class="empty-state-icon" aria-hidden="true">🐾</div><h3>Sin perfiles en esta categoría</h3><p>Consulta directamente con ARCY para conocer la disponibilidad más reciente.</p><a class="button button--outline" href="https://wa.me/525523298138" target="_blank" rel="noopener noreferrer">Consultar por WhatsApp</a></div></div>`;
        return;
      }
    ...
    render(activeChip?.dataset.filter || "all");
  }
